...
export function initAsciiPlayer(options: AsciiPlayerOptions) {
  ...
  function updateRenderState(meta: Manifest) {
    ...
    lastRenderedFrameIndex = -1; // Force redraw after state update
  }
  ...
  function drawFrame(idx: number, meta: Manifest) {
    if (!renderState || idx === lastRenderedFrameIndex) return;
    const frame = frames[idx];
    if (!frame) return;

    const {
      width, height, dpr, offsetX, offsetY, lineHeight, scaleX, fillStyle, shadowColor, font
    } = renderState;
    ...
    // Reuse rowBuffer to avoid allocations
    if (rowBuffer.length !== cols) rowBuffer.length = cols;
